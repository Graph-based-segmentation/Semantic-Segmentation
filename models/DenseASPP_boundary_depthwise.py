...
class DenseASPP_boundary_depthwise(nn.Module):
    ...
    * output_scale can only set as 8 or 16
    ...
    def __init__(self, model_cfg, n_class=19, output_stride=8):
        ...
        self.ASPP_9 = _DenseAsppBlock(input_num=num_features + d_feature1 * 1, num1=d_feature0, num2=d_feature1,
        ...
        self.ASPP_18 = _DenseAsppBlock(input_num=num_features + d_feature1 * 1, num1=d_feature0, num2=d_feature1,
                                       dilation_rate=18, drop_out=dropout0, bn_start=True)

        self.ASPP_24 = _DenseAsppBlock(input_num=num_features + d_feature1 * 1, num1=d_feature0, num2=d_feature1,
                                       dilation_rate=24, drop_out=dropout0, bn_start=True)

        self.conv1x1 = nn.Conv2d(in_channels=(num_features + d_feature1) * 3, out_channels=432,
    ...
    def forward(self, _input):
        ...
        mk_feature3 = torch.cat((feature, mk_aspp3), dim=1)
        mk_aspp6 = self.ASPP_6(mk_feature3)
        mk_feature6 = torch.cat((feature, mk_aspp6), dim=1)
        mk_aspp9 = self.ASPP_9(mk_feature6)
        mk_feature9 = torch.cat((feature, mk_aspp9), dim=1)
        concat_feature1 = torch.cat((mk_feature3, mk_feature6, mk_feature9), dim=1)
        concat_feature1 = self.conv1x1(concat_feature1)

        mk_aspp12 = self.ASPP_12(feature)
        mk_feature12 = torch.cat((feature, mk_aspp12), dim=1)
        mk_aspp18 = self.ASPP_18(mk_feature12)
        mk_feature18 = torch.cat((feature, mk_aspp18), dim=1)
        mk_aspp24 = self.ASPP_24(mk_feature18)
        mk_feature24 = torch.cat((feature, mk_aspp24), dim=1)
        concat_feature2 = torch.cat((mk_feature12, mk_feature18, mk_feature24), dim=1)
        concat_feature2 = self.conv1x1(concat_feature2)

        concat_feature3 = torch.cat((concat_feature1, concat_feature2), dim=1)
        ...
        cls = self.classification(concat_feature3)
