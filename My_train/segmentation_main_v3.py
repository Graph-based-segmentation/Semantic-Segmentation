...
# from models.DenseASPP_boundary import *
...
def main():
    ...
    for epoch in range(curr_epoch, args.num_epochs + 1):
        ...
        train(train_loader, net, criterion, optimizer, epoch, args, total_num_paramters)

    torch.save(net.state_dict(), os.path.join(ckpt_path, 'Model', ImageNet, exp_name_ImageNet,
                                              'model-{}'.format(total_num_paramters) + '.pkl'))
    print('Training Done!!')

def train(train_loader, net, criterion, optimizer, epoch, train_args, total_num_paramters):
